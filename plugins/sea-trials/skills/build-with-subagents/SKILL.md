...
description: >-
  ...
  integration, agent-validate, then delegate to /pre-push-harden and
  optional /pr-review-loop-worktree. Use when the user wants build +
...
| Push-ready gates only | `/pre-push-harden` |
| PR thread resolution | `/pr-review-loop-worktree` or `-inplace` |
...
When unsafe, run **`/pr-review-loop-worktree`** Phase 0 worktree setup
...
Launch **one parent turn** per parallel wave. Cursor: use the **Task**
tool (`generalPurpose` only). Claude: use **Agent** / `context: fork`.
...
**Stop and invoke `/pre-push-harden` now.** Load that skill and run
every phase through READY.

Do not inline gate logic here — `pre-push-harden` runs `pnpm` scripts
that parallelize internally.

Sea Trials: never use `/create-pr skip-checks`; use
`pnpm pr-review-push` or `/pre-push-harden`.
...
| State | Action |
| --- | --- |
| PR exists | Invoke `/pr-review-loop-worktree` (default) or `-inplace` |
...
| --- | --- | --- |
| Build shard worker | `generalPurpose` | Default for all shards |
| Gate / CI | Parent shell | Run `pnpm` gate scripts — no Task fan-out |
| Review fix | Review-loop skill | After push, via `$ST_REVIEW*` |
