---
name: jira-to-pr-loop
description: >-
  Given a Jira ticket key or URL, fetch context via Atlassian MCP,
  resolve the linked GitHub PR, then invoke /pr-review-loop-worktree
...
Invoke **`/pr-review-loop-worktree`** (default) with prompt prefix:
...
- Bare `git push`
- Skipping `/pre-push-harden` before push
